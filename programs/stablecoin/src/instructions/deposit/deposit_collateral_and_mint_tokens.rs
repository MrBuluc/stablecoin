use crate::pyth_config::{get_feed_id_from_hex, PriceUpdateV2};
use crate::{
...
pub struct DepositCollateralAndMintTokens<'info> {
    ...
    #[account(init_if_needed, payer = depositor, space = 8 + Collateral::INIT_SPACE, seeds = [SEED_COLLATERAL_ACCOUNT, depositor.key().as_ref()], bump)]
    pub collateral_account: Account<'info, Collateral>,

    #[account(mut, seeds = [SEED_SOL_ACCOUNT, depositor.key().as_ref()], bump)]
